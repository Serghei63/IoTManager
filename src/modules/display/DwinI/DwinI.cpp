    ...
    void uartHandle() {
        ...
        while (_myUART->available()) {
            ...
            if (_headerIndex >= 2 && _headerIndex == (_headerBuf[2] + 2)) {
                ...
                // 1. Системное событие переключения страницы от DWIN (VP 0x0084)
                if (vpAddr == "0084") {
                    uint16_t page = (_headerBuf[7] << 8) | _headerBuf[8];
                    generateOrder("dwin_page", String(page));
                }
                // 2. Кнопки навигации по сайдбару (VP 0x1000)
                else if (vpAddr == "1000") {
                    uint8_t dataStart = (_headerBuf[3] == 0x83) ? 7 : 6;
                    uint16_t targetPage = (_headerBuf[dataStart] << 8) | _headerBuf[dataStart + 1];

                    if (_debug) {
                        SerialPrint("I", "DwinI", "Nav Button Click -> Target Page: " + String(targetPage));
                    }

                    // Переключаем экран
                    std::vector<IoTValue> p;
                    IoTValue pageVal;
                    pageVal.valD = targetPage;
                    pageVal.isDecimal = true;
                    p.push_back(pageVal);
                    execute("setPage", p);

                    // Уведомляем систему
                    generateOrder("dwin_page", String(targetPage));

                    // Подгружаем историю (для страниц 1..4 в каналы 0..3)
                    if (targetPage >= 1 && targetPage <= 4) {
                        drawHistoryToChannel(targetPage - 1, targetPage - 1);
                    }
                }
                // 3. Обработка всех остальных переменных
                else {
                    uint8_t dataStart = (_headerBuf[3] == 0x83) ? 7 : 6;
                    uint16_t rawVal = (_headerBuf[dataStart] << 8) | _headerBuf[dataStart + 1];
                    String valStr = String(rawVal);
