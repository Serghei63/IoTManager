...
#include "NTP.h"
#include <SdFat.h> // Перешли на продвинутую библиотеку SdFat
// БЫЛО: SdFat sd;
// СТАЛО:
extern SdFat sd; // Говорим компилятору, что реальный объект sd лежит в основном коде
// Инициализируем объект SdFat. 
// В основном файле (например, в setup) нужно будет вызвать: sd.begin(CHIP_SELECT_PIN, SD_SCK_MHZ(24))
//SdFat sd; 
...
public:
    SDLoging(String parameters) : IoTItem(parameters)
    {
        ...
        SerialPrint("I", F("SDLoging"), "'" + id + "' создан модуль логирования на SdFat (LFN)");
    }
    ...
    void doByInterval()
    {
        ...
        // Создание директорий средствами SdFat
    ...
    void SetDoByInterval(String valse)
    {
        ...
        if (!isTimeSynch) return;

        regEvent(value, F("SDLogingEvent"));
    ...
    // Создание нового файла (флаг усечения O_TRUNC обнуляет файл, если он существовал)
    void createNewFileWithData(String &logData)
    {
logData = logData + ",";
    // БЫЛО: String path = _logDir + "/" + String(unixTimeShort) + ".txt";
    // СТАЛО:
    String path = _logDir + "/" + String(unixTime) + ".txt"; // Имя файла теперь полноценный timestamp (2026 год)

    FsFile file = sd.open(path.c_str(), O_WRONLY | O_CREAT | O_TRUNC);
        if (!file) {
            SerialPrint("E", F("SDLoging"), "'" + id + "' Ошибка создания файла на SdFat: " + path);
            return;
        }
        ...
        {
            SerialPrint("E", F("SDLoging"), "'" + id + "' Ошибка записи пути в DB");
            return;
        }
       
        SerialPrint("i", F("SDLoging"), "'" + id + "' Файл успешно создан (LFN) -> " + path);
    }

    // Дозапись в конец файла с помощью O_APPEND
    void addNewDataToExistingFile(String &path, String &logData)
    {
        ...
        if (!file) {
            SerialPrint("E", F("SDLoging"), "'" + id + "' Ошибка дозаписи в файл на SdFat: " + path);
            return;
        }
        ...
        SerialPrint("i", F("SDLoging"), "'" + id + "' Точка успешно добавлена -> " + path);
    }
    ...
    void sendSDFileToWsByFrames(String path, String page, String topicJson, int wsNum) {
        ...
        if (!file) {
            SerialPrint("E", F("SDLoging"), "Ошибка открытия файла для WS: " + path);
            return;
        }

        String startJson = "{\"maxCount\":" + String(calculateMaxCount()) + ",\"topic\":\"" + mqttRootDevice + "/" + id + "\",\"status\":[";
    ...
    void publishValue()
    {
        ...
        if (filePath == "failed" || filePath == "" || !sd.exists(filePath.c_str())) {
            SerialPrint("i", F("SDLoging"), "Нет данных на SD для отправки графиков");
            clearValue();
            return;
        }

        SerialPrint("i", F("SDLoging"), "Выгрузка истории с SdFat: " + filePath);
        
        if (_publishType == TO_WS || _publishType == TO_MQTT_WS || _publishType == -2) 
        {
            String jsonHeaders = getAdditionalJson();
            sendSDFileToWsByFrames(filePath, "charta", jsonHeaders, _wsNum);
            SerialPrint("i", F("SDLoging"), "История успешно улетела в WebSocket -> " + filePath);
        }
    }
    ...
    unsigned long getFileUnixLocalTime(String path) { 
        ...
        // Просто парсим число из имени файла, strtoul безопаснее для больших таймстампов
    unsigned long rawTime = strtoul(selectToMarkerLast(deleteToMarkerLast(path, "."), "/").c_str(), NULL, 10);
    ...
    void setValue(const IoTValue &Value, bool genEvent = true)
    {
        ...
        regEvent(value.valS, "SDLoging", false, genEvent);
    }
...
{
    if (subtype == F("SDLoging"))
    {
        return new SDLoging(param);
    }
    return nullptr;
}
    */
